...
本模块是Q2项目的核心分析模块，实现了基于BMI的孕妇分组和最佳NIPT检测时点优化。采用纵向建模和网格搜索优化，确保结果的稳健性和临床可解释性。
